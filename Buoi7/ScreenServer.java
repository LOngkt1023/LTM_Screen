		...
		try {
			...
			while (true) {
				try {
					...
					// Đọc thông tin tốc độ từ client (non-blocking)
					if (dis.available() >= 8) { // 8 bytes cho double
						double clientSpeed = dis.readDouble();
						System.out.println("Client " + soc.getRemoteSocketAddress() + 
										 " speed: " + (int)clientSpeed + " KB/s");
					}
			...
			System.out.println("Client disconnected: " + soc.getRemoteSocketAddress());
		}
	}
}
